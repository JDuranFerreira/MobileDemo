...
| Direction | With |
|---|---|
| Reads | [`IInputService`](input-service.md), [`Economy`](economy.md), [`Level`](level.md), `TowerCatalogue`, `TowerDefinition.Cost` |
...
| Constructed by | [`Bootstrap`](bootstrap.md), in `Start` rather than `Awake` — see Gotchas |

## Data

No serialized fields — it is a plain class. Three numbers arrive as constructor arguments from
`GameConfig`: `SellRefundFraction`, and (through `PlacementRules`) `BuildRoadClearance` and
`BuildTowerSpacing`. §7 records why the refund fraction is a run rule rather than a per-tower one.
...
- **A sold tower is deactivated, never destroyed.** With destroy-and-recreate, the stack
  ...
  GameObject lingers, inactive, for the rest of the round, owned by the undo stack.
- **Nothing clears the stack**, because there is no phase boundary to clear it on. That is the
  interim rule, not the intended one: `BuildState.Enter`/`Exit` is where it belongs, and it is also
  what will finally destroy those inactive towers.
...
- **A tower placed at runtime can silently never fire.** `ProjectileFactory` is prewarmed once, so
  its projectile prefab must have been collected at boot — which is what `TowerCatalogue` is for
...
- **Building is never blocked**, so §10's "no `Instantiate` during a wave" is violated by one
  allocation per tap. Bounded, named, and it repairs itself when `BuildState` lands (§9).

## Status

**Implemented and fully unit-tested; one link unproven on screen.** `BuildControllerTests` (17),
`PlaceTowerCommandTests` (8), `SellTowerCommandTests` (7) and `PlacementRulesTests` (13) pass as
part of a 205-test suite, and the whole dispatch — tap, hit-test, validate, choose a command, push,
undo — runs through `FakeInputService` with no scene.
...
| Clearing the stack, and destroying sold towers | `BuildState.Exit()` (§4) |
